...
export function Feed() {
  const { stories, hasCertifiedToday } = useApp();
  const [currentTime, setCurrentTime] = useState(Date.now());
  const [reactions, setReactions] = useState<Record<string, string | null>>({});

  const certified = hasCertifiedToday();
          ...
          {stories.map((story) => (
            <div key={story.id} className="bg-white rounded-3xl overflow-hidden border-2 border-neutral-200">
              ...
              <div className="px-5 py-4 flex items-center justify-between">
                <div>
                  <p className="text-neutral-900 font-semibold">{story.userName}</p>
                ...
                <div className="px-3 py-1.5 bg-orange-50 rounded-full flex items-center gap-1.5">
                  ...
                  <p className="text-orange-600 text-xs font-medium">{getTimeRemaining(story.expiresAt)}</p>
                </div>
              </div>
